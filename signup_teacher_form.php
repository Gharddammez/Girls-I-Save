			<form id="signin_teacher" class="form-signin" method="post">
					...
					<select name="department_id" class="input-block-level span12">
						<option></option>
						<?php
                        $query = mysqli_query($conn, 'select * from department order by department_name ') or exit(mysqli_error());
			...
			jQuery("#signin_teacher").submit(function(e){
					...
					$.ajax({
						...
						if(html=='true')
						{
						$.jGrowl("Welcome to Girls-I-Save Learning Management System", { header: 'Sign up Success' });
						var delay = 1000;
